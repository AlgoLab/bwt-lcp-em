Experimental Evaluation {#sec:exp-eval}
=======================
...
`extLCP` [@Cox2016] (BEETL), as well as a more
recent version (BEETL2) that implements a fully external
memory approach[^1], an in-memory method
`gsa-is` [@DBLP:journals/tcs/LouzaGT17], and two recent external memory
tools (`egsa` and `egap`). Unless explicitly stated otherwise, we have
...
(a) 148bp Illumina reads from the Genome In A Bottle (GIAB) [@giab2014]
...
Since the Illumina dataset have a 300$\times$ mean coverage, it is sure
...
dependency on $l$ (see
Section [\[sec:complexity\]](#sec:complexity){reference-type="ref"
reference="sec:complexity"}). Since the other datasets that we have
...
256GB solid state disk. The machine is equipped with 8GB of RAM. In the
...
Main memory usage {#sec:main-memory-usage}
-----------------
